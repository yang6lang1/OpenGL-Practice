...
public class PlayWithColors {
  ...
  public static final String strFragmentShader = "#version 330\n"+
	  ...
	  "void main()\n"+
	  "{\n"+
	  "    float lerpValueX = gl_FragCoord.x / 800.0f;\n"+
	  "    vec4 outputColorX = mix(vec4(1.0f, 1.0f, 1.0f, .5f),\n"+
	  "    vec4(0.7f, 0.2f, 0.6f, .5f), lerpValueX);\n"+
	  "    float lerpValueY = gl_FragCoord.y / 600.0f;\n"+
	  "    vec4 outputColorY = mix(vec4(1.0f, 1.0f, 1.0f, .5f),\n"+
	  "    vec4(0.2f, 0.2f, 0.9f, .5f), lerpValueY);\n"+
	  "    float average = (lerpValueX + lerpValueY) /2.0f;\n"+
	  "    outputColor = mix(outputColorX, outputColorY, average);\n"+
	  "}\n"
	  ;
